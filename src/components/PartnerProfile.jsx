            ...
            selectedMethod ? (() => {
              ...
              return (
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 12, background: opt?.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 700, color: opt?.color }}>
                    {opt?.symbol}
                  </div>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 17 }}>{opt?.label}</div>
                    {opt?.hasHandle && paymentHandle && (
                      <div style={{ fontSize: 13, color: "#666", marginTop: 2 }}>{paymentHandle}</div>
                    )}
                  </div>
                </div>
              );
            })() : (
              <p style={{ color: "#888", margin: 0 }}>No payment method set. Click Change to add one.</p>
            )
          ) : (
            <div>
                ...
                {PAYMENT_OPTIONS.map((opt) => (
                  <label
                    ...
                    style={{
                      ...
                      borderRadius: 12, border: `2px solid ${selectedMethod === opt.value ? opt.color : "#eee"}`,
                      background: selectedMethod === opt.value ? opt.bg : "#fafafa", cursor: "pointer"
                    }}
                  >
                    <input type="radio" name="paymentMethod" value={opt.value} checked={selectedMethod === opt.value} onChange={() => setSelectedMethod(opt.value)} />
                    <div style={{ width: 40, height: 40, borderRadius: 10, background: opt.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 700, color: opt.color }}>
                      {opt.symbol}
                    </div>
              ...
              {PAYMENT_OPTIONS.find((o) => o.value === selectedMethod)?.hasHandle && (
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 12, color: "#666", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 6 }}>
                    {PAYMENT_OPTIONS.find((o) => o.value === selectedMethod)?.handleLabel}
                  </div>
                  <input
                    type="text"
                    value={paymentHandle}
              ...
              <button
                ...
                style={{ padding: "10px 20px", borderRadius: 10, border: "none", background: selectedMethod ? "#121212" : "#ccc", color: "#fff", fontWeight: 600, cursor: selectedMethod ? "pointer" : "not-allowed" }}
              >
