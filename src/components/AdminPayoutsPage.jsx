...
const AdminPayoutsPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubs = partners.map((p) =>
      onSnapshot(
        query(
          collection(db, "partners", p.id, "payouts"),
  ...
  const handleBulkCreate = async () => {
    const eligible = partners.filter(
      (p) => !hasPayoutThisMonth(p.id) && getDefaultAmount(p.id) > 0,
    );
    if (eligible.length === 0) {
      setMsg("All partners already have a payout for this month.");
      return;
    }
    if (
      !window.confirm(
        `Create payouts for ${eligible.length} partner(s) for ${currentMonth}?`,
      )
    )
      return;
    setBulkSaving(true);
    ...
    try {
      await Promise.all(
        eligible.map(async (p) => {
          ...
          await logPayoutEvent(p.id, "payout-created", {
            month: currentMonth,
            amount,
            subscriberCount: subscriberCounts[p.id] || 0,
          });
  ...
  const handleDeletePayout = async (partnerId, payoutId) => {
    const payout = (payoutsByPartner[partnerId] || []).find(
      (p) => p.id === payoutId,
    );
    ...
    try {
      ...
      if (payout)
        await logPayoutEvent(partnerId, "payout-deleted", {
          month: payout.month,
  ...
  const handleMarkPaid = async (partnerId, payoutId) => {
    ...
    try {
      ...
      if (payout)
        await logPayoutEvent(partnerId, "payout-paid", {
          month: payout.month,
          amount: payout.amount,
          subscriberCount: payout.subscriberCount,
        });
    ...
    <div style={{ maxWidth: 1100, margin: "60px auto", padding: "0 20px" }}>
      ...
      >
        ...
        <p style={{ margin: 0, color: "#d6d6d6" }}>
          Create, adjust, and track partner payouts across all locations.
        </p>
        <div
          style={{
            display: "grid",
      ...
      >
        ...
        >
          ...
          >
            {bulkSaving
              ? "Creating..."
              : `➕ Create All Payouts for ${currentMonth}`}
          </button>
          {msg && (
            <span
              style={{
                fontSize: 13,
                fontWeight: 600,
                color: msg.startsWith("✓") ? "#1a7f37" : "#dc3545",
              }}
            >
              {msg}
            </span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {partners.map((partner) => {
          ...
          const partnerTotalPaid = payouts
            .filter((p) => p.status === "paid")
            .reduce((s, p) => s + (p.amount || 0), 0);
          const partnerTotalOwed = payouts
            .filter((p) => p.status === "pending")
            .reduce((s, p) => s + (p.amount || 0), 0);
          ...
          return (
            ...
            >
              ...
              {!alreadyThisMonth && (
                <div
                  style={{
                    ...
                    alignItems: "center",
                    flexWrap: "wrap",
                    marginBottom: payouts.length > 0 ? 16 : 0,
                    padding: "12px 14px",
                    background: "#f8f5ea",
                    borderRadius: 10,
                  }}
                >
                  <span
                    style={{ fontSize: 13, color: "#8a6a00", fontWeight: 600 }}
                  >
                    This month:
                  </span>
                  <div
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        await addDoc(
                          collection(db, "partners", partner.id, "payouts"),
                          {
                            month: currentMonth,
                        ...
                        await logPayoutEvent(partner.id, "payout-created", {
                          month: currentMonth,
                          amount: defaultAmount,
                          subscriberCount: subCount,
                        });
              ...
              {alreadyThisMonth && (
                <div
                  style={{
                    marginBottom: payouts.length > 0 ? 12 : 0,
                    fontSize: 13,
                    color: "#1a7f37",
                    fontWeight: 600,
                  }}
                        ...
                        {p.status === "pending" ? (
                          <>
                            ...
                            >
                              ...
                              <input
                                ...
                                onBlur={(e) => {
                                  ...
                                  if (val !== p.amount)
                                    handleUpdateAmount(partner.id, p.id, val);
                                }}
                                style={{
                                  width: 72,
                                  padding: "4px 6px",
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                handleDeletePayout(partner.id, p.id)
                              }
                              style={{
                                padding: "5px 10px",
                                background: "#fff",
